...
fn isADuck(possible_duck: anytype) bool {
    ...
    const walks_like_duck = @hasDecl(MyType, "waddle"); // possible_duckがwaddle()メソッドを持っているか
    const quacks_like_duck = @hasDecl(MyType, "quack"); // possible_duckがquack()メソッドを持っているか
