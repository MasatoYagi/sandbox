...
pub fn main() void {
    ...
    // and we'll produce a delicious meal. 素材から選ばせるスタイル
    //
    ...
    // Look at each Food on the menu... それぞれのメニューに対してループ
    const meal = food_loop: for (menu) |food| { // food_loopというラベルを付ける

        // Now look at each required ingredient for the Food... それぞれの必要素材に対してループ
        for (food.requires, 0..) |required, required_ingredient| { // 0..でインデックスを取得

            // This ingredient isn't required, so skip it. 必要ない素材はスキップ
            ...
            // required ingredient list for each food.) 顧客が欲しい素材かどうかを確認
            ...
            if (!found) continue :food_loop; // food_loopからcontinue
        }
        ...
        else break food; // food_loopからfoodを返す
    } else menu[0]; // デフォルトのメニューを返す
