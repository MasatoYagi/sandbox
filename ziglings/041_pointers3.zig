...
pub fn main() void {
    ...
    var p: *u8 = undefined; // p、p.*ともに変更可能
    // var p: *const u8 = undefined; // pは変数で変更可能だが、参照先のインスタンスは変更できない(p.* += 1 ができない)
    // const p: *const u8 = undefined; // pが定数であるため、ポインタを変更できない(p = &foo ができない)
