...
fn printTuple(tuple: anytype) void {
    ...
    //
    inline for (fields) |field| { // inline forでないとコンパイルエラー(values of type '[]const builtin.Type.StructField' must be comptime-known, but index value is runtime-known)になる
