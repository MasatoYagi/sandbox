...
pub fn main() void {
    ...
    const zen12: *const [21]u8 = "Memory is a resource."; // バイト配列(文字列)のポインタ
    //
    ...
    // const zen12_string: []const u8 = zen_manyptr; // 修正前
    const zen12_string: []const u8 = zen_manyptr[0..21]; // ポインタをスライスに変換(0..ではなく0..21にする必要がある。0..だと長さが不明なため、[*]const u8型になってしまう)
