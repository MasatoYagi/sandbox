...
pub fn main() void {
    ...
    const base1: []const u8 = scrambled[15..23]; // stringsのバイト配列は定数で、スライスにする時も定数である必要がある！
...
fn printPhrase(part1: []const u8, part2: []const u8, part3: []const u8) void { // スライスの型の指定は[]const u8である必要がある！([]u8ではダメ)
